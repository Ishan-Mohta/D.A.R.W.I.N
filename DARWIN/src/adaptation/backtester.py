...
class Backtester:
    ...
    def _should_rebalance(self, current: pd.Timestamp,
        ...
        if freq == 'W':
            return current.isocalendar()[1] != last.isocalendar()[1] or \
        ...
        for i, date in enumerate(trading_days):
            ...
            for agent in self.agents:
                ...
                for asset, sig in signals.items():
                    ...
                    if action == 'BUY':
                        # If agent didn't specify a value, use a sensible default
                        if desired_value <= 0:
            ...
            if self._should_rebalance(date, last_rebalance):
                ...
                # Adapt params BEFORE redistribution so agents see metrics
                # from the period that just ended.
                for agent in self.agents:
